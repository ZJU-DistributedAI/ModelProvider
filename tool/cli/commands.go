...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	sub = &cobra.Command{
		Use:   `model-provider ["/model/askData/HASH/ETH_KEY/TRANSID"]`,
		Short: ``,
	...
	tmp2 := new(UploadModelProviderCommand)
	sub = &cobra.Command{
		Use:   `model-provider ["/model/upload/HASH/ETH_KEY/HE_KEY/RSA_KEY"]`,
		Short: ``,
	...
	tmp3 := new(UploadFinalModelProviderCommand)
	sub = &cobra.Command{
...
func (cmd *AskDataModelProviderCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/model/askData/%v/%v/%v", url.QueryEscape(cmd.Hash), url.QueryEscape(cmd.ETHKey), cmd.TransID)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
...
func (cmd *AskDataModelProviderCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	...
	var hash string
	cc.Flags().StringVar(&cmd.Hash, "hash", hash, `data hash`)
	var transID int
	cc.Flags().IntVar(&cmd.TransID, "transID", transID, `ask data for transaction[ID]`)
}
...
func (cmd *UploadModelProviderCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/model/upload/%v/%v/%v/%v", url.QueryEscape(cmd.Hash), url.QueryEscape(cmd.ETHKey), url.QueryEscape(cmd.HEKey), url.QueryEscape(cmd.RSAKey))
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
...
func (cmd *UploadModelProviderCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	...
	var hash string
	cc.Flags().StringVar(&cmd.Hash, "hash", hash, `encrypted model IPFS address`)
}
